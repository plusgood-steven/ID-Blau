...
class Trainer():
    ...
    def _train_epoch(self):
        ...
        for idx, sample in enumerate(tq):
            ...
            loss = self.criterion_l1(output, sharp) + self.criterion_fft(output, sharp)
            loss.backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--end_epoch", default=1300, type=int)
    ...
    parser.add_argument("--criterion", default='l1+fft', type=str)
    ...
    parser.add_argument("--dir_path", default='./experiments/FFTformer_Realblur_J', type=str)
    parser.add_argument("--model_name", default='FFTformer_Realblur_J', type=str)
    ...
    Train_set = RealBlur_Loader(data_path=train_data_path, mode="train", crop_size=args.crop_size)
    ...
    # Val loader
    Val_set = RealBlur_Loader(data_path=args.data_path, mode="test", crop_size=args.crop_size)
    dataloader_val = DataLoader(Val_set, batch_size=args.batch_size, shuffle=True, num_workers=8,
    ...
    if dist.get_rank() == 0:
        ...
        writer = SummaryWriter(os.path.join("FFTformer_log", args.model_name))
